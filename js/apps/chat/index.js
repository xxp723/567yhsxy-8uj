...
/* =========================================================
 * 【区域标记：按键图标图案区】
 * 涉及到的所有按键图标图案统一引用 IconPark 字节跳动开源网站里的图标图案。
 * 方便后续统一修改。
 * ========================================================= */
export const ICONS = {
...
/* =========================================================
 * 【区域标记：持久化存储区】
 * 所有涉及持久化存储的代码，请使用项目中的 db.js 模块（基于 IndexedDB）。
 * 禁止使用 localStorage 或 sessionStorage。
 * 不要保留任何 localStorage 的读写逻辑，也不要写双份存储的兜底代码。
 * ========================================================= */
...
/* =========================================================
 * 【区域标记：开关 UI 生成区】
 * 如有单独的开关按键，除了有特殊说明的以外，请参考 iPhone 手机的滑动开关。
 * 注意与主题 UI 风格相统一。
 * ========================================================= */
export function renderIphoneSwitch(isOn = false) {
  return `
    <div class="chat-iphone-switch ${isOn ? 'is-on' : ''}" style="width:50px;height:30px;border-radius:15px;background:${isOn ? '#34C759' : '#E9E9EA'};position:relative;transition:background 0.3s;cursor:pointer;">
      <div class="chat-iphone-switch-knob" style="width:26px;height:26px;border-radius:50%;background:#FFF;position:absolute;top:2px;left:${isOn ? '22px' : '2px'};transition:left 0.3s;box-shadow:0 3px 8px rgba(0,0,0,0.15);"></div>
    </div>
  `;
}
...
/* =========================================================
 * 【区域标记：弹窗与提示区】
 * 如有涉及到弹窗的地方，都不要用原生浏览器弹窗，统一使用本应用内弹窗，与主题UI风格相统一。
 * ========================================================= */
...
export async function mount(container, context) {
  ...
  // 初始化结构
  container.innerHTML = `
    <div class="chat-app" id="chat-app-root" style="width: 100%; height: 100%; display: flex; flex-direction: column;">
      <div class="chat-content" id="chat-content" style="flex: 1; overflow: hidden; position: relative;"></div>
      <div class="chat-tabbar" id="chat-tabbar" style="height: 60px; flex-shrink: 0; background: var(--bg-color, #fff); border-top: 1px solid var(--border-color, #eee);"></div>
    </div>
  `;
  ...
  try {
    ...
    console.error('闲谈应用数据初始化失败:', error);
    openModal(rootEl, {
      title: '应用加载失败',
      body: `<div style="padding:20px;text-align:center;color:var(--text-color);">读取数据失败，请重试。<br><small style="color:gray;">${esc(error.message || String(error))}</small></div>`,
      hideFooter: true
    });
    // 数据异常，中断后续渲染，只返回空卸载函数
    return () => {};
  }
