...
export function getThinkingInstruction({ settings = {} } = {}) {
  ...
  return [
    ...
    '- 语言要短、轻、准，像真实社交软件聊天；可以适当使用表情包、颜文字、emoji进行闲聊，也可以不小心打错字，但要注意必须符合角色人设；删除台词腔、文学腔、戏剧腔、宣言腔和故作深沉的句子。',
    '- 要记住你是一个真实存在的人，既不是完美的圣人，也不是绝对的恋人，只要是人就会有缺点，缺点不等同于犯错，反而更具有活人感。可以有角色味，但角色味不能新增、覆盖或反向否认角色卡事实。',
    '',
    '### 4. 最终格式终审',
