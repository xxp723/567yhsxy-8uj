...
除非指令自己明确要求使用或禁止某种特定格式。`,
    ...
    /* --------------------------------------------------------------------------
       ...
       -------------------------------------------------------------------------- */
    /* --------------------------------------------------------------------------
       [功能规则·已更新·本次短句标点生硬修复：线上聊天与气泡规则]
       说明：
       1. 明确“协议块边界”只依赖 [类型] 角色名：内容 与下一协议块，不依赖句末句号。
       2. 已强化短句、语气词、emoji、颜文字不补句号，避免模型把格式自检误解成每句都要以句号收尾。
       3. 前端解析已按协议块/自然断句防掉格式，不需要用句号兜底；本区只调整提示词约束，不改持久化存储。
       -------------------------------------------------------------------------- */
    `# 线上聊天输出硬性规则（必须遵守）
...
5. 每个文字气泡只放一句话，必须使用 ${CHAT_PROTOCOL_REPLY_FORMAT}；多句话拆成多个协议块，禁止长段落、编号列表、气泡序号或说明文字；协议块边界由“[类型] 角色名：内容”和下一条协议块识别，绝不依赖句号兜底。
6. 角色名填写当前聊天对象名称；内容只放真正要显示给用户的话。
7. 句末标点必须自然口语化：短句、语气词、emoji、颜文字不要为了显得完整而补句号，例如“啊”“嗯”“好”“哈哈”“🥺”“QAQ”可直接结束；只有本来就需要停顿或完整表达的长句才正常使用逗号/句号/问号/叹号，禁止用空格硬替代必要标点。
8. 正式输出前自检 ${protocolChecklistText} 完整协议块数量与格式；自检只检查协议头、角色名、冒号、内容和块数，不要求每个内容都以句号结尾，不合格就后台重写。`,
