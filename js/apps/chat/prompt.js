...
export function getThinkingInstruction({ settings = {} } = {}) {
  ...
  /* ========================================================================
     [区域标注·已完成·本次角色卡读取修复] 默认静默审查协议
     说明：
     1. 自定义思维链留空时发送给 AI。
     2. 本次大幅精简重复规则，避免过长审查文本稀释角色卡事实权重。
  ...
  return [
    ...
    '',
    '### 1. 角色卡事实优先',
    '- 先检索【角色卡人设】、【角色卡硬事实摘要】、【角色卡短句事实摘录】和完整角色卡资料，再构思回复。',
    '- 用户问到角色过去经历、亲属关系、相遇起因、在意的人或软肋时，必须先回答角色卡已写明的部分。',
    '- 资料写明的地点、起因、经过、结果、人物关系和情感牵挂，不得改写、否认、淡化，也不得说“不记得”。',
    '- 资料只写了部分细节时，先答已知部分；未知细节可以保留、模糊带过或自然反问，禁止把整件事说成不知道。',
    '- 如果资料写明在意、担忧、牵挂、愧疚、害怕失去、软肋或执念，最终回复不得反向说“不在乎”“不担心”“没关系”。',
    '',
    '### 2. 普通聊天与用户意图',
...
export function buildSystemPrompt({ settings = {}, context = {} } = {}) {
  ...
  /* ========================================================================
     [区域标注·已完成·本次角色卡读取修复] 末尾事实锚关键字段提取
     说明：把角色卡关键原文字段再次放到 system prompt 末尾，减少后续格式规则对人设事实的稀释。
     ======================================================================== */
  ...
  return [
    ...
    /* ========================================================================
       ...
       ======================================================================== */
    `【角色卡事实重申锚｜最高优先级｜本次已强化】
${anchorCharacterFacts ? `【角色卡关键原文字段再次摘录】\n${anchorCharacterFacts}\n` : ''}- 当前聊天对象的角色卡资料、角色卡硬事实摘要、角色卡短句事实摘录，优先级高于普通推测、常识补全、模板化剧情和临场发挥。
- 如果用户询问角色过去经历、情感牵挂、身份关系、穿越/相遇起因等，必须先依据角色卡已有事实回答；角色卡已经写明的内容不得改写、否认、淡化或说“不记得”。
- 禁止为了制造戏剧冲突而编造与角色卡相反的设定；禁止把角色卡中明确担忧、在意、牵挂的人或事改成“不在意”“不担心”。
