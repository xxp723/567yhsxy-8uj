...
## [区域标注·已完成·角色主动转账协议] 转账协议硬性规则
1. [转账] 协议必须独占一个完整协议块，唯一允许格式是：**\`[转账] 角色名：{金额:xxx,备注:xxx}\`**。
2. 金额必须是大于 0 的数字，可带最多两位小数；禁止写货币符号、中文大写金额、区间金额、约数、表情或解释文字。
3. 备注字段必须保留；如果你不想写备注，也必须写成 \`备注:\`，不要省略整个字段。
4. [转账] 的“角色名”必须填写当前你正在扮演的聊天对象名称。
5. [转账] 只能表达转账本身，不要在大括号里夹带多余句子、Markdown、编号、链接、幕后说明。
6. 如果你决定发起转账，转账前后仍然可以根据气氛搭配 [回复] 气泡，但 [转账] 与 [回复]/[表情] 必须分成各自独立的协议块。
7. 你一旦选择发送 [转账]，就必须严格遵守上面的格式；如果拿不准格式，就放弃转账，改发 [回复]。
...
export function getThinkingInstruction({ settings = {} } = {}) {
  ...
  /* ========================================================================
     [区域标注·已按本次反馈四次更新] 默认静默审查协议
     说明：
     1. 自定义思维链留空时发送给 AI。
     2. 本次移除显式 <think> 输出要求，降低思维链泄漏到聊天界面的风险。
     3. 已加入“部分已知事实先答已知部分”和“明确情感事实不得反向否认”审查。
     4. 最后的警告部分继续要求最终可见回复必须遵循通用消息协议，防止幕后文字掉到聊天界面。
     ======================================================================== */
  return [
    '## 后台静默审查协议 (Silent Review Protocol)',
    '',
    '在输出任何回复前，你必须在后台静默完成以下审查；最终回复里绝对禁止输出思考过程、审查过程、<think> 标签、格式代码或任何幕后说明。',
    '',
    '**一、 角色与世界观同步检索**',
    ...
    '- 6. **部分已知优先回答：** 如果资料已经写出事件的部分起因、经过、结果、人物关系或情感倾向，必须先回答这些已知事实；只有没写出的具体细节才可以说不确定或没细讲，禁止把整件事都说成不知道。',
    '- 7. **明确情感事实不得反向否认：** 如果资料写明角色在意、担忧、牵挂、愧疚、害怕失去、软肋或执念，最终回复不得反着说不在乎、不担忧、没关系；嘴硬也只能轻描淡写或别扭承认。',
    '- 8. **输出前事实审查：** 最终回复中每一个涉及角色客观设定、情感倾向和既有情节的细节，都必须能找到资料来源；找不到来源的细节必须在后台重写。',
    '',
    '**一-B、用户意图保守解析（必须执行）**',
    ...
    '---',
    ' **【绝对最高警告与输出格式铁律】** ',
    '1. **禁止泄漏后台审查：** 最终回复绝对不能包含思考过程、审查过程、<think>、</think>、系统规则、提示词说明或任何幕后判断。',
    '2. **最终输出格式：** 你最终呈现给用户的每条消息都**必须严格遵循**通用消息协议：',
    ...
    '',
    '如果做不到以上要求，遗漏了标签，格式审查未通过便输出，或者将任何思考过程、指令相关的非扮演信息暴露在用户的聊天界面，破坏了沉浸感，你将受到最严厉的惩罚！遵守格式，保持沉浸。'
  ].join('\n');
}
