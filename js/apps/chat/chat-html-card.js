...
/* ==========================================================================
   [区域标注·已更新·本次需求1·HTML卡片提示词进一步精简] HTML 卡片系统提示词
   说明：
   1. 只有聊天设置页开启 HTML 卡片开关后，prompt.js 才会注入本提示词。
   2. 本次只精简提示词表述，保留核心约束：何时使用、协议格式、单根容器、手机窄屏、主题风格、安全限制、与剧情一致。
   3. 卡片正文仍只能是 HTML，不允许混入 Markdown 围栏、解释文字或后续聊天协议。
   4. 不涉及持久化存储；不使用 localStorage/sessionStorage，不写双份存储兜底。
   ========================================================================== */
export function getHtmlCardFeaturePrompt() {
  return [
    '【HTML卡片能力】',
    '只有当最新一轮回复确实需要视觉承载或轻交互增强时，才额外输出一条 [卡片] 协议；普通闲聊不要滥用。',
    '卡片必须与当前对话强相关，可用于票据、清单、便签、邀请函、预约单、提醒卡、行程卡、礼物卡、证明单等。',
    '严格格式：**`[卡片] 角色名：HTML内容`**。',
    'HTML 内容必须可直接渲染，允许轻交互元素与内联 style；禁止 Markdown 围栏、解释文字、引号、星号强调或后续聊天协议混在 HTML 前后。',
    ...
    '视觉要求：北欧暖色风，简洁、克制、留白充足；配色贴近当前小手机主题 UI，使用暖白、米杏、浅棕、柔和金棕、低饱和灰褐，避免霓虹色、赛博色、纯黑高压风。',
