...
/* ==========================================================================
   [区域标注·已完成·本次旁白循环动作与剧情过渡修复] 旁白称呼锁定与连续性修复
   说明：
   1. 只在旁白模式开启时调用，注入到 system prompt。
   2. 退出旁白模式后不再发送此提示词。
   3. 旁白使用 [旁白]...[/旁白] 标记，前端提取后居中加粗显示。
   4. 本区已完成旁白称呼锁定、循环动作抑制与剧情过渡修复：旁白生成前必须优先读取最近一轮用户消息、AI 可见回复、上一轮旁白与最近心声里的地点、姿态、动作、已完成事件和线上/线下状态，再在该锚点上顺推。
   5. 本次已强化第二人称称呼：当用户选择“第二人称（你）”时，旁白正文里所有指代用户的内容必须写成“你”；历史旁白、用户面具姓名、后台标签里的用户名/第三人称都不能继续沿用为本轮旁白称呼。
   6. 本次已明确禁止循环动作：像进门、进屋、走到身后、靠近、坐下、下车这类已完成位移动作，只能作为当前状态继续承接，不能在后续轮次重复重演。
   7. 已明确禁止无依据跳场景、跳地点、回退已完成动作，也禁止把历史后台标签里的“用户”误写成旁白正文称呼；未改动存储、弹窗、图标或其它聊天逻辑。
   ========================================================================== */
export function buildAsideModeSystemPrompt(asideSettings = {}, context = {}) {
  const settings = normalizeAsideSettings(asideSettings);
  const roleName = String(context.roleName || '角色').trim() || '角色';
  const userName = String(context.userName || '用户').trim() || '用户';

  const roleRef = settings.rolePerson === 'first' ? '我' : roleName;
  const userRef = settings.userPerson === 'second' ? '你' : userName;
  const userLabelRule = settings.userPerson === 'second'
    ? `旁白正文里凡是指代用户的内容，称呼必须固定为“你”。即使用户面具姓名是“${userName}”，即使历史旁白曾写过用户名、第三人称或“用户/对方/他/她/TA”，本轮旁白也必须全部改写为“你”，禁止再写成“${userName}”“用户”“对方”“她/他/TA”或其它替代称呼。输出前必须自检：如果旁白正文中出现指代用户的用户名、第三人称或泛称，就先改成“你”再输出。`
    : `旁白正文里对用户的称呼必须固定为“${userName}”，禁止改写成“用户”“对方”“她/他/TA”或其它替代称呼。`;
  const historyLabelRule = settings.userPerson === 'second'
    ? '“用户：”“你：”这类字样在历史里只是后台消息标签，不是旁白正文可直接照搬的称呼；当前已选择第二人称，因此旁白正文中出现用户姓名、用户名或第三人称时必须改写成“你”。'
    : '“用户：”“你：”这类字样在历史里只是后台消息标签，不是旁白正文可直接照搬的称呼；除非用户档案姓名本身就叫“用户”，否则旁白正文禁止把用户称作“用户”。';
...
4. ${historyLabelRule}
5. ${styleDesc}
6. 旁白字数控制在 ${settings.minWords}-${settings.maxWords} 字。
7. 旁白要简单直白，像小说白描旁白，描述动作、神态、状态，不要有多余的修饰修辞。
8. 示例旁白格式参考（不要照搬内容）：
   - "${roleRef}看完这条消息后，手上的动作没有停，只是顺着刚才的姿势抬眼看了一下屏幕，然后继续给${userRef}回复。"
   - "${roleRef}已经站在${userRef}身后时，呼吸放得更轻了一点，视线落在${userRef}的反应上，没有再重复靠近。"
   - "${roleRef}进屋之后没有再回到门外，只是顺手把门带上，目光仍停在${userRef}身上，接着刚才的节奏往下说。"

## 历史连续性硬规则
1. 生成旁白前必须先读取最近短期历史，优先级为：最近一条旁白 > 最近一条 AI 可见回复 > 最近一条用户消息 > 更早历史；先确认当前地点、姿态、相对位置、正在做的事、已完成动作、线上/线下状态。
2. 最近一轮已写出的地点、姿态、距离和动作结果，默认就是当前已生效状态。本轮必须承接这个状态顺推；没有用户明确推动、足够时间、转场过程或历史证据时，禁止换地点、跳场景、突然完成长距离移动、从线下跳回线上，或把已完成动作改回未发生。
3. “进门、进屋、下车、坐下、走近、走到身后、靠近、伸手碰到”等位移/入场/接触动作，一旦写成已完成，后续轮次只能写其结果和后续微动作，禁止重复再演；除非历史明确写过离开、拉开距离、重新转场，或用户明确要求重新发生。
4. 用户本轮只是聊天、确认、追问或发表情时，只写原场景内的微动作、情绪、视线、屏幕回复反应；禁止每轮重置成新的开场白，尤其不要反复写“从门外走进来”“再次推门而入”“又走到你身后”“重新来到你面前”。
