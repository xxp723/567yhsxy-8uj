...
/* ==========================================================================
   ...
   5. 已明确禁止无依据跳场景、跳地点、回退已完成动作，也禁止把历史后台标签里的“用户”误写成旁白正文称呼。
...
export function buildAsideModeSystemPrompt(asideSettings = {}, context = {}) {
  ...
  const userLabelRule = settings.userPerson === 'second'
    ? '旁白正文里对用户的称呼必须固定为“你”，禁止改写成“用户”“对方”“她/他/TA”或其它替代称呼。'
...
3. ${userLabelRule}
4. “用户：”“你：”这类字样在历史里只是后台消息标签，不是旁白正文可直接照搬的称呼；除非用户档案姓名本身就叫“用户”，否则旁白正文禁止把用户称作“用户”。
5. ${styleDesc}
