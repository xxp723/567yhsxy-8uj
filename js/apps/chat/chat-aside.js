...
/* ==========================================================================
   [区域标注·已完成·本次第二人称称呼强制修复] 旁白称呼锁定与连续性修复
   说明：
   1. 只在旁白模式开启时调用，注入到 system prompt。
   2. 退出旁白模式后不再发送此提示词。
   3. 旁白使用 [旁白]...[/旁白] 标记，前端提取后居中加粗显示。
   4. 本区已完成旁白称呼锁定与连续性修复：旁白生成前必须优先读取最近一轮用户消息、AI 可见回复、上一轮旁白与最近心声里的地点、姿态、动作、已完成事件和线上/线下状态，再在该锚点上顺推。
   5. 本次已强化第二人称称呼：当用户选择“第二人称（你）”时，旁白正文里所有指代用户的内容必须写成“你”；历史旁白、用户面具姓名、后台标签里的用户名/第三人称都不能继续沿用为本轮旁白称呼。
   6. 已明确禁止无依据跳场景、跳地点、回退已完成动作，也禁止把历史后台标签里的“用户”误写成旁白正文称呼。
   7. 旁白描述简单直白，白描手法，无多余修饰修辞；未改动存储、弹窗、图标或其它聊天逻辑。
   ========================================================================== */
...
3. 连续性优先级必须是：最近一条旁白 > 最近一条 AI 可见回复 > 最近一条用户消息 > 更早历史；只要最近一轮已经给出地点、动作、姿态、距离或状态，本轮就必须优先承接，不能无视最近一轮去跳写别的情景。
4. 本轮旁白必须从最近一轮已发生内容自然顺滑推进；没有用户明确推动、时间足够经过、交通/转场过程或历史证据时，禁止突然换地点、突然换场景、突然完成长距离移动、突然从线下回到线上，或把上一轮已经完成的动作改回未发生。
5. 如果上一轮角色在门边、车里、书房、卧室、路上等具体场景，本轮必须默认仍在该场景或其合理下一步；除非历史或本轮用户明确给出转场原因。
6. 如果上一轮写“还在车里/走到门边/已经下车/已经进屋/已经坐下”等状态或完成动作，本轮只能承接其结果继续写，不能倒退成“已经下车后又还在车里”“已经到门边后又回到远处”“已经进屋后又没进屋”。
7. 如果本轮用户只是在聊天、确认、追问或发表情，没有推动线下行动，就只在原场景内写微动作、情绪和屏幕回复反应，不主动开新地点。
8. 历史有冲突时，以最近一轮原文为准；无法判断时宁可保守承接“还在刚才的位置/继续刚才的动作”，不要编造新场景。
